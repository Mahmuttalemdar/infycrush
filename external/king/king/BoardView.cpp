...
namespace King
{
   ...
   const std::string BoardView::SELECTION_CURSOR_PATH = root_path + "/sprites/cursor.png";
   const std::string BoardView::REMOVE_SPRITE_PATH = root_path + "/sprites/highlight.png";
   ...
   BoardView::BoardView(const uint8_t& columns, const uint8_t& rows) :
      ...
      m_GemFactory(std::make_unique<GemFactory>()),
      m_State(State::SelectFirstGem),
      m_SelectionCursor(std::make_unique<Sprite>()),
      m_RemoveHighlight(std::make_unique<Sprite>())
   {
   ...
   {
      m_PosX = static_cast<uint32_t>(x);
      m_PosY = static_cast<uint32_t>(y);
   }
   ...
   void BoardView::MoveGem(Gem* gem, const uint8_t& targetColumn, const uint8_t& targetRow)
   {
      ...
      if (targetRow < m_Rows && targetColumn < m_Columns)
         {
      ...
      if (IsMouseOverBoard() == true)
         {
            ...
            uint8_t xCell = static_cast<uint8_t>( (m_MousePosX - m_PosX) / m_CellWidth );
            uint8_t yCell = static_cast<uint8_t>( (m_MousePosY - m_PosY) / m_CellHeight );
                  ...
                  if (highlightedGem != nullptr)
                     {
                        if (hoveredCell != m_HighlightedCell || highlightedGem->GetState() == Gem::State::Normal)
                           {
                              ResetCell(static_cast<uint8_t>(m_HighlightedCell % m_Columns), static_cast<uint8_t>(m_HighlightedCell / m_Rows));
   ...
   void BoardView::SwapCells(const int& firstCell, const int& secondCell)
   {
      ...
      if (((firstCell / m_Rows) == (secondCell / m_Rows) && abs(firstCell - secondCell) == 1)
         ...
         {
            // invoke gems selected event
            GemsSelected(static_cast<uint8_t>(m_FirstCell % m_Columns )
                         ,static_cast<uint8_t>( m_FirstCell / m_Rows)
                         ,static_cast<uint8_t>( secondCell % m_Columns)
                         ,static_cast<uint8_t>( secondCell / m_Rows));
   ...
   void BoardView::Render()
   {
      ...
      for (int i = 0; i < m_Rows * m_Columns; ++i)
         {
            ...
            if (gem != nullptr)
               {
                  auto* currentSprite = std::move(gem->GetSprite());
      ...
      if (m_State == State::SelectSecondGem)
         {
            auto* selectedSprite = std::move(m_Gems[m_FirstCell]->GetSprite());
