...
namespace King
{
   const std::string Application::ROOT_ASSETS_FOLDER = std::string(ASSETS_PATH);
   const std::string Application::FONT_PATH = ROOT_ASSETS_FOLDER + "/fonts/Roboto-Bold.ttf";
   const std::string Application::BACKGROUND_PATH = ROOT_ASSETS_FOLDER + "/sprites/background.jpg";
   const std::string Application::OVERLAY_PATH = ROOT_ASSETS_FOLDER + "/sprites/overlay.png";
   ...
   bool Application::Initialize()
   {
      ...
      // create a window
      m_Window = SDL_CreateWindow("InfyCrush", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, m_WindowWidth, m_WindowHeight, SDL_WINDOW_OPENGL | SDL_WINDOW_SHOWN);
      if (m_Window == nullptr)
         {
      ...
      m_Background->SetWidth(m_WindowWidth);
      m_Background->SetHeight(m_WindowHeight);
      m_Background->SetPosition(0, 0);

      // create the board controller
      m_BoardController = std::make_unique<BoardController>();
      // init to fill board with gems
      m_BoardController->Initialize();
      m_BoardController->Freeze(true);

      // create and setup a countdown timer
      m_CountdownTimer = std::make_unique<CountdownTimer>(60);
      ...
      m_OverlayBackground = std::make_unique<Sprite>();
      ...
      m_OverlayBackground->SetPosition(0, 0);

      // labels for start screen
      m_WelcomeLabel = std::make_unique<Label>(40);
      m_WelcomeLabel->SetText("Welcome to Infy Crush");
      m_WelcomeLabel->SetColor(0, 60, 80);
      m_WelcomeLabel->SetPosition(50, 100);

      m_StartGameLabel = std::make_unique<Label>(30);
      ...
      while (m_IsRunning)
         {
            ...
            m_DelayTime = static_cast<int32_t>( m_WaitTime - (SDL_GetTicks() - m_FrameStartTime) );
            if (m_DelayTime > 0)
               {
                  SDL_Delay(static_cast<Uint32>(m_DelayTime));
               }
   ...
   std::string Application::GetRootAssetsFolder() const
   {
      return ROOT_ASSETS_FOLDER;
   }
